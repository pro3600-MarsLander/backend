...
class TestEnvironment(unittest.TestCase):
    ...
    def test_collision(self):
        ...
        # Collision : 
        lander = Lander(lander_position.x, lander_position.y, 0, 0, 0, 0, 0)
        environment = Environment(surface, lander)
        environment.step(Action(0, 0))
        self.assertTrue(environment.collision())
        self.assertTrue(environment.lander.crashed)
        self.assertFalse(environment.lander.landed)
        self.assertEqual(environment.lander.position, lander_position)
        self.assertEqual(environment.lander.velocity, Point(0, 0))
        self.assertEqual(environment.lander.fuel, 1000)
        self.assertEqual(environment.lander.angle, 0)
        self.assertEqual(environment.lander.power, 0)
        self.assertEqual(environment.lander.thrust, 0)
